...
const [data,setData]=useState(All_data.data_sci);
...
<div className="container ">
        ...
        <h1 className="elementor-heading-title elementor-size-default p-3">Data Science</h1>
        <p className='p-3 text-start'>One of the most propelling careers on the planet today, Data Science is a vast arena of diverse technologies and techniques. Are you ready to step into the world of Data? Then you probably should not miss this corner of articles that is an assorted curation out of tedious research &amp; exploration.</p>
        <p className='p-3 text-start'>The GUVI Blog's Data Science category is here to help you steer through a lot of opinions that float on the internet. High-Quality content curated by experts!</p>
         ...
         <div>    
          <ol className='list d-flex p5'>
                <Link className='btn btn-dark mx-2' to={'/'}>ALL</Link>
                <Link className='btn btn-dark mx-2' to={'/fsd'}>FULL STACK DEVELOPMENT</Link>
                <Link className='btn btn-dark mx-2' >DATA SCIENCE</Link>
                <Link className='btn btn-dark mx-2' to={'/cyber'}>CYBER SECURITY</Link>
                <Link className='btn btn-dark mx-2' to={'/carrer'}>CAREER</Link>
            </ol>
            </div>
            <div className="container px-2  mt-4">
            <div className="row  row-cols-1 row-cols-md-2 row-cols-xl-3 justify-content-center ">
              
              {data.map((item)=>(<Create_cards key={item.id} {...item} /> ))}
            </div>
            </div>
